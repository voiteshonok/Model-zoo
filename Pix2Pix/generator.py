...
class Block(nn.Module):
    def __init__(self, in_channels, out_channels, down, act="relu", use_dropout=False):
...
class UnetGenerator(nn.Module):
    def __init__(self, in_channels=3, features=64):
        ...
        # using in_channels double time of features because of concationation from encoder
        self.up1 = Block(
...
def test():
    ...
    model = UnetGenerator(in_channels=3, features=64)
    preds = model(x)
    assert preds.shape == torch.Size([1, 3, 256, 256])
